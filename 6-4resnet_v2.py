...
def stack_blocks_dense(net, blocks, outputs_collections = None):
    ...
    for block in blocks:
        with tf.variable_scope(block.scope, 'block', [net]) as sc:
            ...
            net = slim.utils.collect_name_outputs(outputs_collections, sc.name, net)
...
def bottleneck(inputs, depth, depth_bottleneck, stride, outputs_collections = None, scope = None):
    with tf.variable_scope(scope, 'bottleneck_v2', [inputs]) as sc:
        ...
        residual = conv2d_same(residual, depth_bottleneck, 3, stride, scope = scope = 'conv2')
...
def resnet_v2(inputs, blocks, num_classes = None, global_pool = True, include_root_block = True, reuse = None, scope = None):
    ...
    with tf.variable_scope(scope, 'resnet_v2', [inputs], reuse = reuse) as sc:
        ...
        with slim.arg_scope([slim.conv2d, bottleneck, stack_blocks_dense], outputs_collections = end_point_collection):
            net = inputs
...
def resnet_v2_50(inputs, num_classes = None, global_pool = True, reuse = None, scope = 'resnet_v2_50'):
    blocks = [Block('block1', bottleneck, [(256, 64, 1)] * 2 + [(256, 64, 2)], Block('block2', bottleneck, [(512, 128, 1)] * 3 + [(512, 128, 2)], Block('block3', bottleneck, [(1024, 256, 1)] * 5 + [(1024, 256, 2)], Block('block4', bottleneck, [(2048, 512, 1)] * 3)]
    return resnet_v2(inputs, blocks, num_classes, global_pool, include_root_block = True, reuse = reuse, scope = scope)

def resnet_v2_101(inputs, num_classes = None, global_pool = True, reuse = None, scope = 'resnet_v2_101'):
    blocks = [Block('block1', bottleneck, [(256, 64, 1)] * 2 + [(256, 64, 2)], Block('block2', bottleneck, [(512, 128, 1)] * 3 + [(512, 128, 2)], Block('block3', bottleneck, [(1024, 256, 1)] * 22 + [(1024, 256, 2)], Block('block4', bottleneck, [(2048, 512, 1)] * 3)]
    return resnet_v2(inputs, blocks, num_classes, global_pool, include_root_block = True, reuse = reuse, scope = scope)

def resnet_v2_152(inputs, num_classes = None, global_pool = True, reuse = None, scope = 'resnet_v2_152'):
    blocks = [Block('block1', bottleneck, [(256, 64, 1)] * 2 + [(256, 64, 2)], Block('block2', bottleneck, [(512, 128, 1)] * 7 + [(512, 128, 2)], Block('block3', bottleneck, [(1024, 256, 1)] * 35 + [(1024, 256, 2)], Block('block4', bottleneck, [(2048, 512, 1)] * 3)]
    return resnet_v2(inputs, blocks, num_classes, global_pool, include_root_block = True, reuse = reuse, scope = scope)

def resnet_v2_200(inputs, num_classes = None, global_pool = True, reuse = None, scope = 'resnet_v2_200'):
    blocks = [Block('block1', bottleneck, [(256, 64, 1)] * 2 + [(256, 64, 2)], Block('block2', bottleneck, [(512, 128, 1)] * 23 + [(512, 128, 2)], Block('block3', bottleneck, [(1024, 256, 1)] * 35 + [(1024, 256, 2)], Block('block4', bottleneck, [(2048, 512, 1)] * 3)]
    return resnet_v2(inputs, blocks, num_classes, global_pool, include_root_block = True, reuse = reuse, scope = scope)
